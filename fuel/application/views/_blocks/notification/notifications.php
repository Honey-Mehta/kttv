...
function loadNotifications(selectedDate = '') {
    $.ajax({
        url: '<?php echo site_url("college/get_notifications"); ?>', // Controller endpoint
        type: 'GET',
        ...
        success: function (response) {
            ...
            if (response.status === 'success') {
                ...
                if (notifications.length > 0) {
                    ...
                    var selectedDateObj = selectedDate ? new Date(selectedDate) : new Date(); // Convert selected date to Date object
                    var sevenDaysLater = new Date(selectedDateObj);
                    sevenDaysLater.setDate(selectedDateObj.getDate() + 7); // Calculate 7 days from selected date

                    // Group notifications by date
                    $.each(notifications, function (index, notification) {
                        var releaseDate = new Date(notification.release_date);
                        var formattedDate = formatDate(releaseDate); // Convert to DD/MM/YYYY

                        // Determine text color (Red if within next 7 days, Black otherwise)
                        var textColor = (releaseDate >= selectedDateObj && releaseDate <= sevenDaysLater) ? "red" : "black";

                        // Check if PDF exists
                        var pdfLink = notification.pdf ? 
                            '<a href="<?php echo base_url("assets/images/notifications/"); ?>' + notification.pdf + 
                            '" target="_blank" rel="noopener" style="text-decoration:none; color:' + textColor + ';">' + notification.headline + '</a>' :
                            '<a href="#" onclick="return false;" style="color:' + textColor + ';">' + notification.headline + '</a>'; // Redirect to #
